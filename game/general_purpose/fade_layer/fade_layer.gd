...
@export_range( 0, 2 ) var transition_time : float = 0.3
...
func fade_in( instant : bool = false ) -> void:
	...
	var _ret : MethodTweener = fade_tween.tween_method( _set_fade, \
		pixel.material.get_shader_parameter( "r" ), 0.0, transition_time )
	var _xret : int = fade_tween.finished.connect( _on_fade_finished )
...
func fade_out( instant : bool = false ) -> void:
	...
	var fade_tween : Tween = create_tween()
	var _ret : MethodTweener = fade_tween.tween_method( _set_fade, \
		pixel.material.get_shader_parameter( "r" ), 1.0, transition_time )
	var _xret : int = fade_tween.finished.connect( _on_fade_finished )
#
