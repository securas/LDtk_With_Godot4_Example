...
func _process_json_leveldata( json : Dictionary ) -> Dictionary:
	...
	for source_level in json_levels:
		...
		var level_params = {}
		if source_level.has( "fieldInstances" ):
			for field_instance in source_level.fieldInstances:
				level_params[field_instance.__identifier] = field_instance.__value
		data[source_level.identifier]["level_params"] = level_params
	return data
