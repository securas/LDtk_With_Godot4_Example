...
func get_level_params() -> Dictionary:
	if not _map or not _map.single_level: return {}
	if not _map.world_data[_map.single_level_name].has( "level_params" ):
		return {}
	return _map.world_data[_map.single_level_name].level_params
